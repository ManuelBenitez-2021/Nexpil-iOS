...
class MorningViewController1: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.delegate?.navColorChange(value: 0)
    }
                ...
                if let data = response.result.value {
                    ...
                    if result == "true"
                    {
                        ...
                        for obj in datas {
                            ...
                            let starttime = DataUtils.getTimeRange(index: 0)!.components(separatedBy: "-")[0]
                            let endtime = DataUtils.getTimeRange(index: 0)!.components(separatedBy: "-")[1]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        else if cellTypes[indexPath.row].celltype == CellType.time //time label for take medication
        {
            ...
            if Int(hour)! > 12
            {
                ...
                updateTime = String(Int(hour)!) + ":" + cellTypes[indexPath.row].labelName.components(separatedBy: ":")[1] + " am"//cellTypes[indexPath.row].labelName + " am"
            }
...
//        }
    ...
    @objc func addNeededSelect(sender : UITapGestureRecognizer) {
        ...
        let time = formatter.string(from: currentDateTime)
        let index = sender.view!.tag
        let mymedication = patientmedications[index]
        let taketime1 = mymedication.createat.components(separatedBy: " ")[1]
        formatter.dateFormat = "h:mm a"
        let eatentime = formatter.string(from: currentDateTime)
        let medicationHistory = MedicationHistory.init(id: 0, medicationId: mymedication.id, date: time, takeTime: taketime1, eatenTime: eatentime, eatText: "1 tablet - Taken " + eatentime, medicationName: mymedication.medicationname, dayTime: taketime,prescription: 1)
