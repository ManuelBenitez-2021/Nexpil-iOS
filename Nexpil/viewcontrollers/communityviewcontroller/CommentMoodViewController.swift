//
//  CommentMoodViewController.swift
//  Nexpil
//
...
class CommentMoodViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
            ...
            .responseJSON(completionHandler: { response in
                ...
                if let data = response.result.value {
                    ...
                    if result == "true"
                    {
                        ...
                        let mood = (data["moodstate"] as! NSString) as String
                        self.moodText.textColor = UIColor.init(hex: DataUtils.getEndColor()!)
                        switch(mood)
                        {
                        case "1":
                            self.moodText.text = "Very Sad"
                            self.photo.image = UIImage.init(named: "Group 1069")
                        case "2":
                            self.moodText.text = "Sad"
                            self.photo.image = UIImage.init(named: "Group 1070")
                        case "3":
                            self.moodText.text = "Neutral"
                            self.photo.image = UIImage.init(named: "Group 1071")
                        case "4":
                            self.moodText.text = "Happy"
                            self.photo.image = UIImage.init(named: "Group 1072")
                        case "5":
                            self.moodText.text = "Very Happy"
                            self.photo.image = UIImage.init(named: "Group 1073")
                        default:
                            break
                        }
...
extension CommentMoodViewController: OnlyPicturesDataSource {
